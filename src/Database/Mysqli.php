...
use Nosun\Swoole\Contract\DataBase\IDatabase;
use Nosun\Swoole\Log;
...
class MySQLi extends \mysqli implements IDatabase
{
    ...
    protected function tryReconnect($call, $params)
    {
        ...
        for ($i = 0; $i < 2; $i++)
        {
            ...
            if ($result === false)
            {
                ...
                {
                    Log::info(__CLASS__ . " SQL Error", $this->errorMessage($params[0]));
                    return false;
                }
            }
            break;
    ...
    function multi_query($sql)
    {
        ...
        if (!$result) {
            Log::info(__CLASS__ . " SQL Error", $this->errorMessage($sql));
            return false;
        }
